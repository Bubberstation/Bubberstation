...
> {
  ...
  currentVelocityLimit: number = 100;
  ...
  longJumpVelocityLimit: number = 100;
  shortJumpVelocityLimit: number = 200;
  idleVelocity: number = 0;
  accel_up_coeff: number = 1;
  bidirectional: boolean = false;
  no_escape: boolean = false;

  baseLongJumpChancePerSecond: number = 0.005;
  ...
  moveBait(
    ...
    const acceleration_up = -1000 * this.accel_up_coeff;
    // Gravity
    const acceleration_down = 750;
  ...
  fishOnBait(fish: Fish, bait: Bait): boolean {
    ...
    const fishLowerBound = fish.position + fish.height * 1.1;
    const baitLowerBound = bait.position + bait.height * 1.1;
