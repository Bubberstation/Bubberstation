...
export function DeadlineModal(props) {
  ...
  // The number of days before the deadline that this code went live at. For calculating percentage progress towards deadline.
  const dateSpan = 93;
  const fontSize =
    12 + Math.round(((dateSpan - days_remaining) / dateSpan) * 54);

  const [selected, setSelected] = useState(0);

  const windowWidth =
    345 +
    Math.round(((dateSpan - days_remaining) / dateSpan) * 900) +
  ...
  const windowHeight =
    ...
    Math.round(((dateSpan - days_remaining) / dateSpan) * 200);
