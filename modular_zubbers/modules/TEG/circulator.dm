...
/obj/machinery/atmospherics/components/binary/circulator
	...
	icon = 'modular_zubbers/modules/TEG/icons/obj/machines/thermoelectric.dmi'
...
/obj/machinery/atmospherics/components/binary/circulator/Initialize(mapload)
	. = ..()
	AddComponent(/datum/component/simple_rotation,ROTATION_REQUIRE_WRENCH | ROTATION_CLOCKWISE | ROTATION_COUNTERCLOCKWISE)
	...
	if(!is_operational)
		...
		if(!last_pressure_delta)
			...
			if(last_pressure_delta > ONE_ATMOSPHERE) //fast
				...
				SSvis_overlays.add_vis_overlay(src, icon, "circ-run", LIGHTING_ABOVE_ALL, ABOVE_LIGHTING_PLANE, dir)
			else	//slow
				if(mode)
...
/obj/machinery/atmospherics/components/binary/circulator/crowbar_act(mob/living/user, obj/item/I)
	if(TRIGGER_SECONDARY_ACTION)
		return
	if(anchored)
