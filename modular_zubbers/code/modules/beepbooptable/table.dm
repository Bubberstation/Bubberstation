...
/obj/structure/table/optable/proc/ekg(mob/living/carbon/patient)
	...
	if(patient.stat >= HARD_CRIT)
		playsound(computer, 'modular_zubbers/sound/machines/operating_table/flatline.ogg', 5, falloff_distance = 5, ignore_walls = FALSE, extrarange = SHORT_RANGE_SOUND_EXTRARANGE)
		most_severe = TRUE
	if(patient.stat == SOFT_CRIT || BLOOD_VOLUME_SURVIVE > patient.blood_volume || organ_fatal_test(patient) && !most_severe)
		playsound(computer, pick('modular_zubbers/sound/machines/operating_table/ekg_alert.ogg', 'modular_zubbers/sound/machines/operating_table/flatline.ogg'), 5, falloff_distance = 5, ignore_walls = FALSE, extrarange = SHORT_RANGE_SOUND_EXTRARANGE)
		most_severe = TRUE
	for(var/datum/wound/wound in patient.all_wounds && !most_severe)
		if(wound.severity >= WOUND_SEVERITY_MODERATE)
			playsound(computer, 'modular_zubbers/sound/machines/operating_table/quiet_double_beep.ogg', 5, falloff_distance = 5, ignore_walls = FALSE,  extrarange = SHORT_RANGE_SOUND_EXTRARANGE)
			most_severe = TRUE
			break
	if(patient.health <= patient.maxHealth && !most_severe)
		playsound(computer, 'modular_zubbers/sound/machines/operating_table/quiet_beep.ogg', 5, falloff_distance = 5, ignore_walls = FALSE,  extrarange = SHORT_RANGE_SOUND_EXTRARANGE)
