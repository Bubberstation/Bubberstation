...
/mob/living/simple_animal/hostile/floor_cluwne
	...
	var/interest = 0
	var/target_area
	var/invalid_area_typecache = list(/area/space, /area/lavaland, /area/mine, /area/centcom, /area/shuttle/syndicate, /area/centcom/interlink)
	var/eating = FALSE
...
/mob/living/simple_animal/hostile/floor_cluwne/Initialize(mapload)
	...
	access_card.access = REGION_ALL_GLOBAL//THERE IS NO ESCAPE
...
/mob/living/simple_animal/hostile/floor_cluwne/proc/Kill(mob/living/carbon/human/H)
	...
	if(do_after(src, 5 SECONDS, H))
		...
		if(prob(75))
			...
			H.cluwneify()
			H.adjust_brute_loss(30)
			H.set_organ_loss(ORGAN_SLOT_BRAIN, 100)
			H.cure_blind(EYES_COVERED)
