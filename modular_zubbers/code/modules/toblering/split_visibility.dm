// Hi, hello, you might have noticed that this is from the wallening. Just because folk didn't like the state it was in, doesn't mean all that hard work has to go to waste.
// Big thanks to lemoninthedark and actioninja for doing some SERIOUS heavy lifting.
// Still not happy about the sheer amount of icon states required, but frankly, it's the most accurate solution.
...
/proc/get_splitvis_object(z_offset, icon_path, junction, dir, color, pixel_x = 0, pixel_y = 0, plane = WALL_PLANE, layer = CLOSED_TURF_LAYER, cache = TRUE)
...
/proc/generate_joined_wall(icon_path, junction, color, draw_darkness = TRUE)
	...
	if(draw_darkness)
		overlays += mutable_appearance('modular_zubbers/icons/turf/walls/wall_blackness.dmi', "blackness", layer = FLOAT_LAYER - 1, appearance_flags = TILE_BOUND | RESET_COLOR | RESET_ALPHA | RESET_TRANSFORM)
...
/datum/element/split_visibility/Attach(datum/target, icon_path, color)
	...
	// target_atom.add_overlay(mutable_appearance('icons/turf/walls/wall_blackness.dmi', "wall_background", BELOW_CLOSED_TURF_LAYER, target_atom, GAME_PLANE))
	// // We draw a copy to the wall plane so we can use it to mask byond darkness, that's all
	// target_atom.add_overlay(mutable_appearance('icons/turf/walls/wall_blackness.dmi', "wall_background", BELOW_CLOSED_TURF_LAYER, target_atom, WALL_PLANE))
	// Ensures when you try to click on a turf, you actually click on the turf, and not the adjacent things holding it
	target_atom.add_overlay(mutable_appearance('modular_zubbers/icons/turf/walls/wall_blackness.dmi', "wall_clickcatcher", WALL_CLICKCATCH_LAYER, target_atom, GAME_PLANE))
...
/datum/element/split_visibility/proc/apply_splitvis_objs(turf/target_turf, junction, add_to_turfs = TRUE)
	...
	for(var/direction in GLOB.cardinals)
		...
		if(add_to_turfs)
			...
			if(HAS_TRAIT(operating_turf, TRAIT_CONTAINS_SPLITVIS))
				...
				vis = get_splitvis_object(offset, icon_path, junction, direction, color, -DIR_TO_PIXEL_X(direction), -DIR_TO_PIXEL_Y(direction))
	...
	for(var/direction in GLOB.diagonals)
		...
		if(!add_to_turfs)
			// I HATE the code duping, but we need to try both to ensure it's properly cleared
			var/mutable_appearance/split_vis/vis
			vis = get_splitvis_object(offset, icon_path, "innercorner", direction, color, 0, 0, HIDDEN_WALL_PLANE, ABOVE_TURF_LAYER)
			target_turf.overlays -= vis
			vis = get_splitvis_object(offset, icon_path, "innercorner", direction, color, -DIR_TO_PIXEL_X(direction), -DIR_TO_PIXEL_Y(direction), layer = ABOVE_TURF_LAYER)
			operating_turf.overlays -= vis
			continue
		var/mutable_appearance/split_vis/vis
		// If we're trying to draw to something with splitvis, just draw to yourself, and use the hidden wall plane
...
/datum/element/split_visibility/Detach(atom/target)
	// target.cut_overlay(mutable_appearance('icons/turf/walls/wall_blackness.dmi', "wall_background", BELOW_CLOSED_TURF_LAYER, target, GAME_PLANE))
	// // We draw a copy to the wall plane so we can use it to mask byond darkness, that's all
	// target.cut_overlay(mutable_appearance('icons/turf/walls/wall_blackness.dmi', "wall_background", BELOW_CLOSED_TURF_LAYER, target, WALL_PLANE))
	// Ensures when you try to click on a turf, you actually click on the turf, and not the adjacent things holding it
	target.cut_overlay(mutable_appearance('modular_zubbers/icons/turf/walls/wall_blackness.dmi', "wall_clickcatcher", WALL_CLICKCATCH_LAYER, target, GAME_PLANE))
