//Credit for the code goes to JohnFulpWillard, main PR appears to be https://github.com/fulpstation/fulpstation/pull/217.
//Body camera sprite was made by Greniza, https://github.com/fulpstation/fulpstation/pull/347.
...
/obj/item/clothing/suit/jacket/officer/Initialize(mapload)
	. = ..()
	AddComponent(/datum/component/bodycamera_holder)
...
/obj/item/bodycam_upgrade
	...
	icon = 'modular_zubbers/code/modules/bodycameras/bodycamera.dmi'
	icon_state = "bodycamera"
	desc = "A body camera that can be attached to an armor vest, there's an instructions tag if you look a little closer..."
	///The camera itself.
...
/datum/component/bodycamera_holder/proc/on_examine_more(atom/source, mob/user, list/examine_list)
	...
	if(bodycamera_installed)
		examine_list += span_notice("It has a bodycamera_installed installed.")
	else
