...
/mob/living/carbon/human/species/protean/Initialize(mapload)
	...
	var/obj/item/organ/stomach/to_remove = get_organ_slot(ORGAN_SLOT_STOMACH) //No free parts for Proteans.
	QDEL_NULL(to_remove)

	var/obj/item/organ/heart/to_remove = get_organ_slot(ORGAN_SLOT_HEART) //No free parts for Proteans.
	QDEL_NULL(to_remove)
