...
/obj/item/clothing/suit/armor/wizardblood/equipped(mob/living/carbon/human/user, slot)
	. = ..()
	if(slot == ITEM_SLOT_BELT && iscarbon(user) && user.reagents)
		START_PROCESSING(SSobj,src)
		owner = user

/obj/item/clothing/suit/armor/wizardblood/dropped(mob/user, silent)
	. = ..()
	STOP_PROCESSING(SSobj, src)
	owner = null

/obj/item/clothing/suit/armor/wizardblood/process(seconds_per_tick)

	if(!owner || !owner.reagents)
		STOP_PROCESSING(SSobj, src)
		return

	for(var/obj/item/reagent_containers/cup/endless_flask/flask as anything in atom_storage.contents) //Only flasks are allowed in these, so...
		var/desired_amount = amount_to_inject
		var/reagent/found_reagent = locate(flask.reagent_to_create) in reagent_list
		if(found_reagent)
