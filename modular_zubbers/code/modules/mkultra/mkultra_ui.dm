...
/datum/action/item_action/organ_action/velvet/ui_data(mob/user)
	. += list("erp_pref" = owner.client?.prefs?.read_preference(/datum/preference/toggle/erp/hypnosis))
	return

/datum/action/item_action/organ_action/velvet/ui_static_data(mob/user)
	var/list/data = list()
