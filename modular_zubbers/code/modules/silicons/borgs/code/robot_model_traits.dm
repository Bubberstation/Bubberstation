...
/obj/item/robot_model/proc/update_robot_rest()
	...
	if (!istype(robot))
		return
	if (model_features && ((TRAIT_R_SQUADRUPED in model_features) || (TRAIT_R_WIDE in model_features) || (TRAIT_R_TALL in model_features) || (TRAIT_R_CANREST in model_features)))
