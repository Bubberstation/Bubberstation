/datum/action/cooldown/spell/pointed/healing_light
	name = "Healing Light"
	desc = "Heals a small amount of damage from the selected target, moving will break your concentration and disturb the grimoire's energy for a short duration, people with a peaceful mind or bearing a healing relic empower this spell."
...
/datum/action/cooldown/spell/pointed/healing_light/cast(mob/living/cast_on)
	...
	var/mob/living/carbon/human/self = owner
	if(!istype(self))
		return FALSE
...
// If a normal pacifist, heal more.
	...
	if(do_after(owner, 1.5 SECONDS, TRUE, IGNORE_TARGET_LOC_CHANGE, target = cast_on))
		...
		span_warning("A wreath of gentle light washes over [cast_on]!"),
		)
		if(need_mob_update)
