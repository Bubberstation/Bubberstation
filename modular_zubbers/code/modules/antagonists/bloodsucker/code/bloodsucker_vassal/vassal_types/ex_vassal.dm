...
/datum/antagonist/vassal/ex_vassal
...
/datum/antagonist/vassal/ex_vassal/on_gain()
	. = ..()
	RegisterSignal(owner.current, COMSIG_ATOM_EXAMINE, PROC_REF(on_examine))

/datum/antagonist/vassal/ex_vassal/on_removal()
	if(revenge_vassal)
...
/datum/antagonist/vassal/ex_vassal/proc/on_examine(datum/source, mob/examiner, examine_text)
	SIGNAL_HANDLER
...
/datum/antagonist/vassal/ex_vassal/add_team_hud(mob/target)
 ...
 */
/datum/antagonist/vassal/ex_vassal/proc/return_to_fold(datum/antagonist/vassal/revenge/mike_ehrmantraut)
...
/datum/antagonist/vassal/ex_vassal/proc/on_life(datum/source, seconds_per_tick, times_fired)
	SIGNAL_HANDLER
	...
	owner.remove_antag_datum(/datum/antagonist/vassal/ex_vassal)
...
/datum/reagent/blood/bloodsucker/expose_mob(mob/living/exposed_mob, methods, reac_volume, show_message, touch_protection)
	var/datum/antagonist/vassal/ex_vassal/former_vassal = exposed_mob.mind.has_antag_datum(/datum/antagonist/vassal/ex_vassal)
	if(former_vassal)
