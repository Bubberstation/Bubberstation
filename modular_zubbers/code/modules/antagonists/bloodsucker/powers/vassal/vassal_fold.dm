...
/datum/action/cooldown/bloodsucker/ghoul_blood/can_use(mob/living/carbon/user, trigger_flags)
	...
	if(!.)
		return FALSE
	var/datum/antagonist/ghoul/revenge/revenge_ghoul = owner.mind.has_antag_datum(/datum/antagonist/ex_ghoul)
	if(revenge_ghoul)
		return FALSE
	...
	if(owner.pulling && isliving(owner.pulling))
		...
		var/datum/antagonist/ex_ghoul/former_ghoul = pulled_target.mind.has_antag_datum(/datum/antagonist/ex_ghoul)
		if(!former_ghoul)
...
/datum/action/cooldown/bloodsucker/ghoul_blood/ActivatePower(trigger_flags)
	. = ..()
	var/datum/antagonist/ghoul/revenge/revenge_ghoul = owner.mind.has_antag_datum(/datum/antagonist/ghoul/revenge)
	if(trigger_flags & TRIGGER_SECONDARY_ACTION)
	...
	if(target_ref)
		...
		var/datum/antagonist/ex_ghoul/former_ghoul = target.mind.has_antag_datum(/datum/antagonist/ex_ghoul)
