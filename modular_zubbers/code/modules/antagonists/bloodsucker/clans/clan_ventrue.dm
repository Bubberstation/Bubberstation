...
/datum/bloodsucker_clan/ventrue/spend_rank(datum/antagonist/bloodsucker/source, mob/living/carbon/target, cost_rank = TRUE, blood_cost)
	if(!target && !has_enough_abilities())
		...
		to_chat(bloodsuckerdatum.owner.current, span_danger("You can only have up to [VENTRUE_MAX_POWERS] powers, anything further will be ranks to spend on your Favorite Ghoul through a Persuasion Rack."))
		return FALSE
	var/datum/antagonist/ghoul/favorite/ghouldatum = target.mind.has_antag_datum(/datum/antagonist/ghoul/favorite)
	var/datum/antagonist/ghoul/non_favorite = target.mind.has_antag_datum(/datum/antagonist/ghoul)
	if(!ghouldatum && ghouldatum.master != bloodsuckerdatum.owner.current || !IS_BLOODSUCKER(target) && !non_favorite && ghouldatum.master != bloodsuckerdatum.owner.current)
		target.balloon_alert(target, "is not a sired bloodsucker nor your favorite ghoul!")
		return FALSE
	if(!ghouldatum.owner.current.mind)
		target.balloon_alert(target, "ghoul must be awake!")
		return FALSE
	// Purchase Power Prompt
	var/list/options = list()
	for(var/datum/action/cooldown/bloodsucker/power as anything in bloodsuckerdatum.all_bloodsucker_powers)
		if(initial(power.purchase_flags) & GHOUL_CAN_BUY && !(locate(power) in ghouldatum.bloodsucker_powers))
	...
	if(options.len < 1)
		...
		var/choice = tgui_input_list(bloodsuckerdatum.owner.current, "You have the opportunity to level up your Favorite Ghoul. Select a power you wish them to recieve.", "Your Blood Thickens...", options)
		...
		if(!choice || !options[choice])
			...
			return
		// Prevent Bloodsuckers from closing/reopning their coffin to spam Levels.
		if((locate(options[choice]) in ghouldatum.bloodsucker_powers))
			to_chat(bloodsuckerdatum.owner.current, span_notice("You prevent your blood from thickening just yet, but you may try again later."))
			return

		// Good to go - Buy Power!
		var/datum/action/cooldown/bloodsucker/purchased_power = options[choice]
		if(!ghouldatum.BuyPower(purchased_power, ghouldatum.bloodsucker_powers))
	...
	if(IS_GHOUL(target) && IS_BLOODSUCKER(target))
		finish_spend_rank(ghouldatum, cost_rank, blood_cost)
		return
	ghouldatum.ghoul_level++
	finish_spend_rank(ghouldatum, cost_rank, blood_cost)
	var/traits_to_add = list()
	switch(ghouldatum.ghoul_level)
		if(1)
		...
		if(4 to INFINITY)
			...
			if(!bloodsucker_target)
				...
				if(target.mind.has_antag_datum(/datum/antagonist/ghoul/favorite))
...
/datum/bloodsucker_clan/ventrue/interact_with_ghoul(datum/antagonist/bloodsucker/source, datum/antagonist/ghoul/favorite/ghouldatum)
	...
	if(!bloodsuckerdatum.GetUnspentRank() <= 0)
		bloodsuckerdatum.SpendRank(ghouldatum.owner.current)
		return TRUE
	if(bloodsuckerdatum.GetBloodVolume() >= BLOODSUCKER_BLOOD_RANKUP_COST)
		...
		to_chat(bloodsuckerdatum.owner.current, span_warning("Do you wish to spend [BLOODSUCKER_BLOOD_RANKUP_COST] Blood to Rank [ghouldatum.owner.current] up?"))
		...
		if(rank_response == "Yes")
			bloodsuckerdatum.SpendRank(ghouldatum.owner.current, cost_rank = FALSE, blood_cost = BLOODSUCKER_BLOOD_RANKUP_COST)
		return TRUE
	to_chat(bloodsuckerdatum.owner.current, span_danger("You don't have any levels or enough Blood to Rank [ghouldatum.owner.current] up with."))
	return TRUE
