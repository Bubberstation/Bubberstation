...
/datum/antagonist/bloodsucker/proc/HandleHealing(mult = 1)
	...
	if((bruteheal + fireheal) && mult != 0) // Just a check? Don't heal/spend, and return.
		...
		user.adjustBruteLoss(-bruteheal * mult, updating_health = FALSE) // Heal BRUTE / BURN in random portions throughout the body.
		user.adjustFireLoss(-fireheal * mult, updating_health = TRUE)
