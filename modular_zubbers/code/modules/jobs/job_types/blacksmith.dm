...
/obj/item/paper/pamphlet/blacksmith_tutorial
	...
	If you lack a dedicated department room then you can purchase a 'Blacksmithing Workstation Pack' from the cargo order console for your stationary tools, and a 'Blacksmithing Tools Pack' for handheld tools. Failing that, almost all of the above can be crafted using basic wood and iron (except the anvil, which requires plastitanium). Use the crafting menu (the hammer icon -- bottom right of your screen) to take a look.<br>\n\
	<br> \
