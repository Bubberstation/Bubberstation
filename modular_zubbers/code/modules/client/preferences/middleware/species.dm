...
// Remove any invalid quirks by the species whitelist/blacklist
/datum/preference_middleware/species/post_set_preference(mob/user, preference, value)
	...
	for(var/quirk_name as anything in preferences.all_quirks)
		...
		var/list/species_blacklist = GLOB.quirk_species_blacklist[quirk_type]
		if(!length(species_whitelist) || (species_id in species_whitelist) || length(species_blacklist) || (species_id in species_blacklist))
			continue
