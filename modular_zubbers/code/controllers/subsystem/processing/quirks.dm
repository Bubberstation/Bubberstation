...
/datum/controller/subsystem/processing/quirks/proc/OverrideQuirks(mob/living/user, client/applied_client, unique = TRUE)
	...
	for(var/quirk_name in required_quirks)
		...
		quirk.add_to_holder(user, existing_quirk, applied_client, unique = unique)
