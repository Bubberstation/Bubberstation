...
/datum/controller/subsystem/traitor/Initialize()
	...
	for(var/datum/uplink_item/u_item as anything in uplink_items)
		if(!u_item.item || u_item.limited_stock >= 0 || u_item.restricted || u_item.cost > SATCHEL_TC_LIMIT || (u_item.purchasable_from == UPLINK_NUKE_OPS) || (u_item.purchasable_from == UPLINK_CLOWN_OPS) ) //The == is intentional here. I don't want to include nuke op or clown op exclusive items..
			continue
