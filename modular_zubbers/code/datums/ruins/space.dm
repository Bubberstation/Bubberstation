/datum/map_template/ruin/space
	prefix = "_maps/RandomRuins/SpaceRuins/bubberstation/"
	cost = 1
	allow_duplicates = FALSE
	ruin_type = ZTRAIT_SPACE_RUINS
	default_area = /area/space

/datum/map_template/ruin/space/onehalf //Modified from the original ruin. See PR 5983.
	id = "onehalf"
	suffix = "onehalf.dmm"
	name = "Space-Ruin DK Excavator 453"
	description = "An old mining station, at least what's left of it. Despite it's age there's still scavengers picking at the wreckage."

/datum/map_template/ruin/space/infested_frigate //Modified from the original ruin. See PR 5983.
	id = "infested_frigate"
	suffix = "infested_frigate.dmm"
	name = "Space Ruin SYN-C Brutus"
	description = "This wasn't an outbreak, this was a repelled attack."
