...
/datum/sprite_accessory/ears/is_hidden(mob/living/carbon/human/wearer)
	...
	if(((wearer.covered_slots & HIDEHAIR) || (wearer.covered_slots & HIDEEARS)) \
		...
		&& ( \
			(wearer.head && ((wearer.head.flags_inv & HIDEHAIR) || (wearer.head.flags_inv & HIDEEARS)) && !(wearer.head.flags_inv & SHOWSPRITEEARS)) \
			|| (wearer.wear_mask && ((wearer.wear_mask.flags_inv & HIDEHAIR) || (wearer.wear_mask.flags_inv & HIDEEARS)) && !(wearer.wear_mask?.flags_inv & SHOWSPRITEEARS)) \
		))
		return TRUE
