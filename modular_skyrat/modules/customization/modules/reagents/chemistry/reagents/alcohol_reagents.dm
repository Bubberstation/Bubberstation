...
/datum/reagent/consumable/ethanol/velvet_kiss
	...
	chemical_flags = REAGENT_CAN_BE_SYNTHESIZED //BUBBER EDIT
	chemical_flags_skyrat = REAGENT_BLOOD_REGENERATING //BUBBER EDIT
...
/datum/reagent/consumable/ethanol/velvet_kiss/expose_mob(mob/living/exposed_mob, methods, reac_volume)
	if(iszombie(exposed_mob) || isvampire(exposed_mob) || isdullahan(exposed_mob) || ishemophage(exposed_mob)) //Rare races! BUBBER EDIT: adds hemophages
		quality = RACE_DRINK
	else
		quality = DRINK_GOOD
	return ..()

/datum/reagent/consumable/ethanol/velvet_kiss/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, times_fired)  //BUBBER EDIT
	. = ..()
	if(drinker.blood_volume < BLOOD_VOLUME_NORMAL)
		drinker.blood_volume = min(drinker.blood_volume + (1 * REM * seconds_per_tick), BLOOD_VOLUME_NORMAL) //Same as Bloody Mary, as it is roughly the same difficulty to make.  Gives hemophages a bit more choices to supplant their blood levels. END OF BUBBER EDIT
