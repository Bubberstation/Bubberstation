...
/datum/outfit/job/hos/pre_equip(mob/living/carbon/human/affected_mob)
	if(affected_mob.jumpsuit_style != PREF_SUIT) //BUBBER EDIT START - Bunnysuits
		to_chat(affected_mob, span_alertwarning("Lopland Head of Security uniforms don't include alternate variants! You've been equipped with a jumpsuit instead.")) //BUBBER EDIT END - Bunnysuits
		affected_mob.jumpsuit_style = PREF_SUIT
	. = ..()

/datum/outfit/job/warden/pre_equip(mob/living/carbon/human/affected_mob)
	if(affected_mob.jumpsuit_style != PREF_SUIT) //BUBBER EDIT START - Bunnysuits
		to_chat(affected_mob, span_alertwarning("Lopland Warden uniforms don't include alternate variants! You've been equipped with a jumpsuit instead.")) //BUBBER EDIT END - Bunnysuits
		affected_mob.jumpsuit_style = PREF_SUIT
	. = ..()
