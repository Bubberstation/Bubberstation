...
/datum/element/loomable/proc/loom_me(obj/item/source, mob/living/user, atom/target)
	...
	var/skill_modifier = user.mind.get_skill_modifier(/datum/skill/production, SKILL_SPEED_MODIFIER) //BUBBER EDIT
	if(isstack(source))
		var/obj/item/stack/stack_we_use = source
		while(stack_we_use.amount >= required_amount)
			if(!do_after(user, loom_time * skill_modifier, target)) //BUBBER EDIT
				break
			...
			spawning_amount++
			user.mind.adjust_experience(/datum/skill/production, 5) //BUBBER EDIT
