...
/datum/roster/proc/spawn_battle_royale(mob/user)
	...
	for(var/datum/contestant/iter_contestant in active_contestants)
		...
		if(iter_contestant.spawn_this_contestant(iter_spawn))
			...
			RegisterSignal(the_guy, COMSIG_MOB_STATCHANGE, PROC_REF(check_br_elimination_stat))
			RegisterSignal(the_guy, list(COMSIG_LIVING_DEATH, COMSIG_QDELETING), PROC_REF(check_br_elimination_dead))
			successes++
