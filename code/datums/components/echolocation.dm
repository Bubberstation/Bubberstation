...
/datum/component/echolocation/Initialize(echo_range, cooldown_time, image_expiry_time, fade_in_time, fade_out_time, images_are_static, blocking_trait, echo_group, echo_icon, color_path, use_echo = TRUE, show_own_outline = FALSE) // Bubber Edit
	. = ..()
...
/datum/component/echolocation/proc/echolocate()
	if(!COOLDOWN_FINISHED(src, cooldown_last) || stall) // Bubber edit
		return
...
/datum/component/echolocation/proc/show_image(image/input_appearance, atom/input, current_time)
	...
	for(var/mob/living/echolocate_receiver as anything in receivers)
		if(!show_own_outline && echolocate_receiver == input) // Bubber Edit
			continue
