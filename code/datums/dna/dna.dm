...
/datum/dna/proc/generate_unique_features()
	...
	for(var/block_type in GLOB.dna_feature_blocks)
		...
		if(isnull(block.mutant_part ? mutant_bodyparts[block.feature_key] : features[block.feature_key])) // BUBBER EDIT CHANGE - Customization - ORIGINAL: if(isnull(features[block.feature_key]))
...
/datum/dna/proc/initialize_dna(newblood_type = random_human_blood_type(), create_mutation_blocks = TRUE, randomize_features = TRUE)
	...
	if(randomize_features)
		/* BUBBER EDIT REMOVAL BEGIN - We don't really want this. We instead let get_mutant_bodyparts() handle the bodypart randomization on our end, to prevent getting any crazy cross-species features.
		...
		features = species.randomize_features() | features // BUBBER EDIT ADDITION
...
/mob/living/carbon/set_species(datum/species/mrace, icon_update = TRUE, pref_load = FALSE, replace_missing = TRUE, list/override_features, list/override_mutantparts, list/override_markings) // BUBBER EDIT CHANGE - ORIGINAL: /mob/living/carbon/set_species(datum/species/mrace, icon_update = TRUE, pref_load = FALSE, replace_missing = TRUE)
	...
	// BUBBER EDIT ADDITION BEGIN - BODYPARTS AND FEATURES
...
/mob/living/carbon/human/updateappearance(icon_update = TRUE, mutcolor_update = FALSE, mutations_overlay_update = FALSE)
	...
	for(var/block_type in GLOB.dna_feature_blocks)
		...
		if(block_to_apply.mutant_part ? dna.mutant_bodyparts[block_to_apply.feature_key] : dna.features[block_to_apply.feature_key]) // BUBBER EDIT CHANGE - Customization - ORIGINAL: if(dna.features[block_to_apply.feature_key])
