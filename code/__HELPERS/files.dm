	...
	for(var/i in 1 to max_iterations)
		...
		switch(choice)
			...
			if("Download Folder")
				...
				if(confirmation != "Yes")
					continue
				log_admin("[key_name(src)] is downloading [length(comp_flist)] files from [path]") // BUBBER EDIT -- ADDITION
				message_admins("Admin [key_name_admin(src)] is downloading [length(comp_flist)] files from [path]") // BUBBER EDIT -- ADDITION
