...
/obj/item/ai_module/law/Initialize(mapload)
	. = ..()
	if(mapload && (HAS_TRAIT(SSstation, STATION_TRAIT_UNIQUE_AI) || HAS_TRAIT(SSstation, STATION_TRAIT_HOS_AI)) && is_station_level(z) && handle_unique_ai() == SHOULD_QDEL_MODULE) // BUBBER Edit HoS AI station trait
