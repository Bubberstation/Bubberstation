...
/datum/objective/kidnapping/generate_objective(datum/mind/generating_for, list/possible_duplicates)
	...
	for(var/datum/objective/kidnapping/objective as anything in possible_duplicates)
...
/datum/objective/kidnapping/ungenerate_objective()
	victim = null
	dropoff_area = null

/datum/objective/kidnapping/on_objective_taken(mob/user)
	. = ..()
	INVOKE_ASYNC(src, PROC_REF(generate_holding_area))

/datum/objective/kidnapping/proc/generate_holding_area()
...
/datum/objective/kidnapping/generate_ui_buttons(mob/user)
	var/list/buttons = list()
	if(!pod_called)
		buttons += add_ui_button("Call Extraction Pod", "Pressing this will call down an extraction pod.", "rocket", "call_pod")
	return buttons

/datum/objective/kidnapping/ui_perform_action(mob/living/user, action)
	. = ..()
	switch(action)
		if("call_pod")
			if(pod_called)
...
/datum/objective/kidnapping/proc/call_pod(mob/living/user)
	pod_called = TRUE
...
/datum/objective/kidnapping/proc/enter_check(obj/structure/closet/supplypod/extractionpod/source, entered_atom)
	if(!istype(source))
...
/datum/objective/kidnapping/proc/return_victim(mob/living/carbon/human/sent_mob)
