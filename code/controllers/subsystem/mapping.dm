...
/datum/controller/subsystem/mapping/proc/LoadGroup(list/errorList, name, path, files, list/traits, list/default_traits, silent = FALSE, height_autosetup = TRUE)
	...
	SSautomapper.preload_templates_from_toml(files) // BUBBER EDIT ADDITION - We need to load our templates AFTER the Z level exists, otherwise, there is no z level to preload.
	var/turf_blacklist = SSautomapper.get_turf_blacklists(files) // BUBBER EDIT ADDITION - We use blacklisted turfs to carve out places for our templates.

	// load the maps
	for (var/P in parsed_maps)
		var/datum/parsed_map/pm = P
		pm.turf_blacklist = turf_blacklist // BUBBER EDIT ADDITION - apply blacklist
		var/bounds = pm.bounds
	...
	// BUBBER EDIT ADDITION BEGIN - We need to load our templates from cache after our space has been carved out.
	if(!LAZYLEN(errorList))
		SSautomapper.load_templates_from_cache(files)
	// BUBBER EDIT ADDITION END
	if(!silent)
		add_startup_message("Loaded [name] in [(REALTIMEOFDAY - start_time)/10]s!") //BUBBER EDIT CHANGE
	return parsed_maps
