		...
		if(GAME_STATE_STARTUP)
			...
			*/ // SKYRAT EDIT START - DISCORD SPAM PREVENTION
			if(!discord_alerted)
				...
				for(var/channel_tag in CONFIG_GET(str_list/channel_announce_new_game))
					send2chat(new /datum/tgs_message_content("<@&[CONFIG_GET(string/game_alert_role_id)]> Round **[GLOB.round_id]** starting on [SSmapping.current_map.map_name], [CONFIG_GET(string/servername)]! \nIf you wish to be pinged for game related stuff, go to <#[CONFIG_GET(string/role_assign_channel_id)]> and assign yourself the roles."), channel_tag) // SKYRAT EDIT - Role ping and round ID in game-alert
			// SKYRAT EDIT END
