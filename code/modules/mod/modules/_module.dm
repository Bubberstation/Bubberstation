...
/obj/item/mod/module/proc/activate()
	if(!COOLDOWN_FINISHED(src, cooldown_timer))
		...
		return FALSE
	if(((!mod.active || mod.activating) && !(allow_flags & MODULE_ALLOW_INACTIVE)) || !mod.get_charge()) // BUBBER EDIT ADDITION: INACTIVE USE
		balloon_alert(mod.wearer, "unpowered!")
		return FALSE

	// BUBBER EDIT START - No using modules when not all parts are deployed.
