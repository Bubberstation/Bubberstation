...
/obj/item/mod/module/armor_booster/on_activation()
	...
	balloon_alert(mod.wearer, "armour boosted, EVA lost!") //Bubber addition: balloon alerts for boosters (I will probably PR this on tg)
...
/obj/item/mod/module/armor_booster/on_deactivation(display_message = TRUE, deleting = FALSE)
	...
	if(!deleting)
		playsound(src, 'sound/mecha/mechmove03.ogg', 25, TRUE, SHORT_RANGE_SOUND_EXTRARANGE)
		balloon_alert(mod.wearer, "armour retracts, EVA ready!") //Bubber addition: Balloon alerts for boosters
	mod.slowdown += actual_speed_added
	mod.wearer.update_equipment_speed_mods()
	var/list/parts = mod.mod_parts + mod
	for(var/obj/item/part as anything in parts)
