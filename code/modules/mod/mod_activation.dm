...
//BUBBER ADDITION START - Stops suit storage from always dropping on retract or deploy
/// Checks whether an item in suit storage can remain there while swapping to a new outer suit.
...
/obj/item/mod/control/proc/deploy(mob/user, obj/item/part, instant = FALSE)
	...
	if(part_datum.can_overslot)
		...
		if(istype(overslot)) // BUBBER EDIT - check for /obj/item, not /obj/item/clothing - ORIGINAL: if(istype(overslot, /obj/item/clothing))
			...
			transfer_part_to_loc(overslot, part, force = TRUE, preserve_suit_storage = can_preserve_suit_storage(part, wearer.s_store)) // BUBBER EDIT - added preserve_suit_storage arg
...
/obj/item/mod/control/proc/retract(mob/user, obj/item/part, instant = FALSE)
	...
	//wearer.transferItemToLoc(part, src, force = TRUE) //BUBBER REMOVAL
	//BUBBER EDIT START - added preserve_suit_storage arg
