...
/obj/effect/mob_spawn/ghost_role/attack_ghost(mob/dead/observer/user)
	...
	// BUBBER EDIT START - ghost cafe observer bypass
	var/started_as_observer = user.started_as_observer || istype(src, /obj/effect/mob_spawn/ghost_role/robot/ghostcafe) || istype(src, /obj/effect/mob_spawn/ghost_role/human/ghostcafe)
	if(!prompt_fail && started_as_observer && allow_custom_character && (GLOB.species_prototypes[species_pref].inherent_respiration_type & RESPIRATION_OXYGEN))
	// BUBBER EDIT END
