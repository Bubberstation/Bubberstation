...
/datum/preference_middleware/loadout/proc/select_item(datum/loadout_item/selected_item)
	...
	if(selected_item.donator_only && !GLOB.donator_list[preferences?.parent?.ckey]) //BUBBER EDIT: Original if((selected_item.donator_only) && !GLOB.donator_list[preferences?.parent?.ckey])
