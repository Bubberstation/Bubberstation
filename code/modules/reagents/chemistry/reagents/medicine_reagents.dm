...
/datum/reagent/medicine/earthsblood/overdose_process(mob/living/affected_mob, seconds_per_tick, times_fired)
	...
	if(iscarbon(affected_mob))
		...
		affected_mob.grant_language(/datum/language/sylvan, source = LANGUAGE_MIND) //BUBBER EDIT: Overdosing Earthsblood lets you permanently speak Sylvan, the precursor language
	if(need_mob_update)
		return UPDATE_MOB_HEALTH
