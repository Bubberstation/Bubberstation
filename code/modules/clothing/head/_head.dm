...
/obj/item/clothing/head/worn_overlays(mutable_appearance/standing, isinhands = FALSE)
	...
	if(damaged_clothes)
		...
		var/damage_icon_state = "damagedhelmet"
		var/mob/living/carbon/human/species_target = loc
		if(ishuman(species_target) && icon_exists('modular_zubbers/icons/effects/item_damage_species.dmi', "damagedhelmet_[species_target.dna.species.id]"))
			damage_icon_file = 'modular_zubbers/icons/effects/item_damage_species.dmi'
			damage_icon_state  = "damagedhelmet_[species_target.dna.species.id]"
