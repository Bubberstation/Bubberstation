...
/datum/antagonist/proc/greet()
	if{(!silent)
		...
		to_chat(owner.current, span_infoplain(span_doyourjobidiot("Do not rush for greentext at all costs. You are not here to play-to-win or powergame. This is not a PvP focused server\
		Do not wordlessly kill, there is an expectation of reasonable conflict escalation here, rather than resorting to mechanics as a first line. It is unacceptable to walk up to somebody and simply stab them.\
		Round Removal is not tolerated here, this includes making a body unreasonably difficult to recover. Please Ahelp if you believe you need to RR somebody. (E.g A person consistently hunting you after death.)"
		)))} //BUBBER EDIT ANTAG SHOULD NOT BE A NUISANCE. PROBABLY
