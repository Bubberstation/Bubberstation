...
/obj/effect/mob_spawn/ghost_role/beno/pre_ghost_take(mob/dead/observer/user)
	...
	new_mob_name = sanitize_name(tgui_input_text(user, "Who are you again?", new_mob_name, max_length = MAX_NAME_LEN))

	new_mob_desc = initial(new_mob_desc)
	new_mob_desc = tgui_input_text(user, "Who are you again?", new_mob_name, max_length = MAX_NAME_LEN)
