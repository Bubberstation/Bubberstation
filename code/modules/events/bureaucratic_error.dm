...
/datum/round_event/bureaucratic_error/start()
	...
	// BUBBER EDIT BEGIN /* BUBBER EDIT REMOVAL START
	if(prob(33)) // Only allows latejoining as a single role.
		...
		return
	// BUBBER EDIT END */ // BUBBER EDIT REMOVAL - no more locking off jobs
	// Adds/removes a random amount of job slots from all jobs.
	for(var/datum/job/current as anything in jobs)
		current.total_positions = max(current.total_positions + rand(-2,4), 1) // BUBBER EDIT CHANGE - No more locking off jobs - ORIGINAL: current.total_positions = max(current.total_positions + rand(-2,4), 0)
