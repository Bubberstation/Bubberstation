...
/datum/unit_test/barsigns_icon
	var/list/blacklisted_sign_types = list(/datum/barsign/skyrat, /datum/barsign/skyrat/large) // BUBBER EDIT ADDITION - Modular barsigns
...
/datum/unit_test/barsigns_icon/Run()
	...
	barsign_icon_states += icon_states(SKYRAT_BARSIGN_FILE) // BUBBER EDIT ADDITION - Need to check modular barsigns
	barsign_icon_states += icon_states(SKYRAT_LARGE_BARSIGN_FILE) // BUBBER EDIT ADDITION - Need to check modular  barsigns

	// Check every datum real bar sign
	for(var/sign_type in (subtypesof(/datum/barsign) - /datum/barsign/hiddensigns))
		// BUBBER EDIT ADDITION BEGIN - MODULAR BARSIGNS
