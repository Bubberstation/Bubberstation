...
/datum/species
	...
	var/list/list/mutant_bodyparts = list() //BUBBER EDIT ADDITION - CUSTOMIZATION (typed list)
...
/datum/species/proc/regenerate_organs(mob/living/carbon/organ_holder, datum/species/old_species, replace_current = TRUE, list/excluded_zones, visual_only = FALSE)
	for(var/slot in get_all_slots())
		...
		if(existing_organ && !disallow_customizable_dna_features) // BUBBER EDIT CHANGE - Though sometimes we might want to do that. - ORIGINAL: if(existing_organ)
