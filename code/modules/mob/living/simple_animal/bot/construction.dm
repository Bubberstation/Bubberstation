...
/obj/item/bot_assembly/cleanbot/attackby(obj/item/item_attached, mob/user, list/modifiers)
	..()
...
/obj/item/bot_assembly/repairbot/attackby(obj/item/item, mob/user, list/modifiers)
	..()
	switch(build_step)
		if(ASSEMBLY_FIRST_STEP)
	...
	switch(build_step)
		if(ASSEMBLY_FIRST_STEP)
			if(istype(W, /obj/item/healthanalyzer))
				var/obj/item/healthanalyzer/analyzer = W // BUBBER EDIT ADDITION BEGIN -- EXTRA ROBOTICS HEALTH ANALYZERS
				if (!analyzer.can_be_used_in_medibot())
					user?.balloon_alert(user, "no attachment ports!")
					return // BUBBER EDIT ADDITION END
				if(!user.temporarilyRemoveItemFromInventory(W))
					return
...
/obj/item/bot_assembly/honkbot/attackby(obj/item/attacking_item, mob/user, list/modifiers)
	..()
	switch(build_step)
		if(ASSEMBLY_FIRST_STEP)
...
/obj/item/bot_assembly/hygienebot/attackby(obj/item/I, mob/user, list/modifiers)
	. = ..()
	var/atom/Tsec = drop_location()
	switch(build_step)
		if(ASSEMBLY_FIRST_STEP)
...
/obj/item/bot_assembly/vim/attackby(obj/item/part, mob/user, list/modifiers)
	. = ..()
	if(.)
		return
	switch(build_step)
