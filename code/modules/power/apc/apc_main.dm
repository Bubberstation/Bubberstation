...
#define APC_CHANNEL_EQUIP_TRESHOLD 25 // SKYRAT EDIT CHANGE - Original: 30
	...
	if(cell && !shorted) //need to check to make sure the cell is still there since rigged/corrupted cells can randomly explode after give().
		...
		if(cell.charge <= 0) // zero charge, turn all off
			...
			if(!nightshift_lights || (nightshift_lights && !low_power_nightshift_lights))
				low_power_nightshift_lights = TRUE
				INVOKE_ASYNC(src, PROC_REF(set_nightshift), TRUE)
		else if(cell.percent() < APC_CHANNEL_EQUIP_TRESHOLD) // turn off equipment // SKYRAT EDIT COMMENT - Changed to turn off lighting instead
			equipment = autoset(equipment, AUTOSET_ON) // SKYRAT EDIT CHANGE - Original: equipment = autoset(equipment, AUTOSET_OFF)
			lighting = autoset(lighting, AUTOSET_OFF) // SKYRAT EDIT CHANGE - Original: equipment = autoset(equipment, AUTOSET_ON)
			environ = autoset(environ, AUTOSET_ON)
			alarm_manager.send_alarm(ALARM_POWER)
			if(!nightshift_lights || (nightshift_lights && !low_power_nightshift_lights))
				low_power_nightshift_lights = TRUE
