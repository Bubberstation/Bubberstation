 ...
 */
/proc/tgui_input_text(mob/user, message = "", title = "Text Input", default, max_length = CAPLESS_MESSAGE_LEN, multiline = FALSE, encode = TRUE, timeout = 0, ui_state = GLOB.always_state) // Bubber edit: max_length = INFINITY
	if (!user)
		user = usr
	if (!istype(user))
