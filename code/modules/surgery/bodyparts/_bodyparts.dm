...
/obj/item/bodypart/proc/receive_damage(brute = 0, burn = 0, blocked = 0, updating_health = TRUE, forced = FALSE, required_bodytype = null, wound_bonus = 0, exposed_wound_bonus = 0, sharpness = NONE, attack_direction = null, damage_source, wound_clothing = TRUE)
	...
	*/
	...
	if(GLOB.global_roster.enable_random_wounds)
		...
		var/bio_state = biological_state
		...
		switch(bio_state)
			...
			if(BIO_BONE)
				...
				if((mangled_state & BODYPART_MANGLED_INTERIOR) && try_dismember(wounding_type, wounding_dmg, wound_bonus, bare_wound_bonus))
					return
			...
			if(BIO_FLESH_BONE)
				...
				if(mangled_state == BODYPART_MANGLED_EXTERIOR && sharpness)
					...
					wounding_type = WOUND_BLUNT
				else if(mangled_state == (BODYPART_MANGLED_EXTERIOR|BODYPART_MANGLED_INTERIOR) && try_dismember(wounding_type, wounding_dmg, wound_bonus, bare_wound_bonus))
					return
