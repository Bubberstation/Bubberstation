...
#define WILDCARD_LIMIT_SILVER list( \
	WILDCARD_NAME_COMMON = list(limit = 5, usage = list()), \ //Bubberstation edit: Original 2
	WILDCARD_NAME_COMMAND = list(limit = 3, usage = list()), \ // Bubberstation edit: original 1
	WILDCARD_NAME_PRV_COMMAND = list(limit = 2, usage = list()) \ //Bubberstation edit: original 1
)
